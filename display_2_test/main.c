...
#define SMALL_IMAGE_LENGHT 18432
...
uint8_t cutted_frame[32][32][2];
uint8_t extended_frame[32][32][3];
uint8_t tensor[1][32][32][3];
...
/*void extractFrame()
{
  for(int i = 0; i < 32; i++)
  {
    ...
    {
      for(int k = 0; k < 2; k++)
        cutted_frame[i][j][k] = frame_buffer[47 + i][47 + j][k];
    }
  }
}*/
...
void core1_capture()
{
 while(1){
     ...
     /* takePicture(&CAM, CAM_IMAGE_MODE_96X96, CAM_IMAGE_PIX_FMT_RGB565);
      while (CAM.receivedLength) 
      { 
        uint32_t rlen = readBuff(&CAM, small_framebuffer, SMALL_IMAGE_LENGHT);
      }*/
